...
var MWLayerAjax = __WEBPACK_IMPORTED_MODULE_0__static_leaflet_leaflet_js___default.a.TileLayer.extend({
	...
	_xhrHandler: function(req, tile, tilePoint) {
		let that = this;
		//		req.tile_request = tile_request[uri];
//		req.seq = req.tile_request.seq;
		req.dest = dest;
//		active_loaders[x.seq] = x;
		return function() {
			...
			if((s >= 200 && s < 300 && s != 204) || s === 304) {
				...
				if((!reg.test(req.responseText)) && req.responseText !== "0") {
					...
					that._tileLoaded(tile, tilePoint);
				}
	...
	_loadTile: function(tile, tilePoint) {
		...
		req.onreadystatechange = this._xhrHandler(req, tile, tilePoint);
...
var MWTileLayer = MWLayerAjax.extend({
	...
	addTileData: function(mwjson, tilePoint) {
		debugger
		//		var features = L.Util.isArray(mwjson) ? mwjson : geojson.features,
		//			i, len, feature;
